class CostService
  ...
  def initialize(classroom_hours_elapsed, fluorescent_units_count, tubes_in_unit, tube_price)
    @classroom_hours_elapsed = classroom_hours_elapsed
  ...
  def calculate
    ...
    @fluorescent_units_count.times do
      total_required_fluorescent_units += single_unit_repair_times(@classroom_hours_elapsed)
    end
  ...
  def single_unit_repair_times(used_hours)
    replace_counter = 0
    while used_hours > 0
      used_hours -= life_of_new_fluorescent_units
      replace_counter += 1
    end
  ...
  def life_of_new_fluorescent_units
